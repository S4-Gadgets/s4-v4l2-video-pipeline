...
sudo dkms add ./s4-tc358748
sudo dkms build s4-tc358748/1.0
sudo dkms install s4-tc358748/1.0

# Install udev rules
sudo cp 99-s4-v4l2.rules /etc/udev/rules.d/
sudo udevadm control --reload-rules

# Install monitor daemon
sudo cp s4-video-monitor.py /usr/local/bin/
sudo cp s4-video-monitor.service /etc/systemd/system/
...
sudo cp s4-video-debug.sh /usr/local/bin/
sudo chmod +x /usr/local/bin/s4-video-debug.sh

# Install and enable debug toggle service and timer
sudo cp s4-debug-toggle.service /etc/systemd/system/
sudo cp s4-debug-toggle.timer /etc/systemd/system/
sudo systemctl daemon-reexec
